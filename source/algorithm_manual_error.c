...
void next_step(double **u, double **unm1, double **unm2, double *x, double *y, double dt,
    ...
    #pragma omp parallel private(t, i, j, thread_id)
    {
        num_thread = (float) omp_get_num_threads();
        thread_id = (float) omp_get_thread_num();
        for(t=0; t<T; t+=dt){
            //Boundary Conditions
            ...
            
            {
                ...
                for(j=(Ny*(thread_id)/num_thread)+1; j<(Ny*(thread_id+1)/num_thread)-1; j++){
                    for(i=1; i<Nx-1; i++){
